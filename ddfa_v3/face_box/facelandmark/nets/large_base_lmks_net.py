...
class InvertedResidual(nn.Module):
    ...
    def __init__(self, inp, oup, stride, padding, use_res_connect, expand_ratio=6):
        ...
        self.conv = nn.Sequential(
            ...
            nn.Conv2d(hid_channels, hid_channels, 3, stride, padding, groups=hid_channels, bias=False),
            nn.BatchNorm2d(hid_channels),
            nn.PReLU(hid_channels),
...
class SoftArgmax(nn.Module):
    ...
    def forward(self, heatmap: torch.Tensor) -> torch.Tensor:
        ...
        if not self.infer:
            ...
            xx, yy = torch.meshgrid(torch.arange(height, device=device), torch.arange(width, device=device), indexing='ij')  # <--- aggiungi questo
...
class LargeBaseLmksNet(nn.Module):
    ...
    def __init__(self, er=1.0, infer=False):
        ...
        self.block15 = InvertedResidual(int(128*er), int(128*er), 1, 1, False, 2)  # [128, 14, 14]
