...
class face_model:
    def __init__(self, args):
        ...
        model = np.load("./ddfa_v3/assets/face_model.npy",allow_pickle=True).item()
        ...
        if args.extractTex:
            #aggiunto io
            self.uv_size = getattr(args, "uv_size", 2048)

            uv_coords_numpy = process_uv(model['uv_coords'].copy(), self.uv_size, self.uv_size)
            self.uv_coords_torch = (torch.tensor(uv_coords_numpy, requires_grad=False, dtype=torch.float32, device=self.device) / (self.uv_size-1) - 0.5) * 2
            if self.device == 'cpu':
                from ddfa_v3.util.cpu_renderer import MeshRenderer_UV_cpu
        ...
        if self.device == 'cpu':
            from ddfa_v3.util.cpu_renderer import MeshRenderer_cpu
        ...
        if args.backbone == 'resnet50':
            ...
            self.net_recon.load_state_dict(torch.load("ddfa_v3/assets/net_recon.pth", map_location=torch.device('cpu'))['net_recon'])
        ...
        if args.backbone == 'mbnetv3':
            ...
            self.net_recon.load_state_dict(torch.load("ddfa_v3/assets/net_recon_mbnet.pth", map_location=torch.device('cpu'))['net_recon'])
            self.net_recon = self.net_recon.to(self.device)
            self.net_recon.eval()
    ...
    def forward(self, no_pca = False):
        ...
        if self.args.ldm68:
            ...
            result_dict['ldm68_inds'] = self.ldm68.detach().cpu().numpy()
        ...
        if self.args.extractTex:
            ...
            #res_colors = np.clip(uv_color_img, 0, 1)

            if no_pca:
                # use fliped img-texture for texture blending
                res_colors = cv2.seamlessClone((uv_color_img*255).astype(np.uint8), cv2.flip((uv_color_img*255).astype(np.uint8), 1), ((1 - uv_weight.detach().cpu().permute(0, 2, 3, 1).numpy()[0])*255)[:,:,0].astype(np.uint8), (self.uv_size//2, self.uv_size//2), cv2.NORMAL_CLONE) / 255.
            else:
                # use pca-texture for texture blending
                res_colors = cv2.seamlessClone((uv_color_img*255).astype(np.uint8), (uv_color_pca*255).astype(np.uint8), ((1 - uv_weight.detach().cpu().permute(0, 2, 3, 1).numpy()[0])*255)[:,:,0].astype(np.uint8), (self.uv_size//2, self.uv_size//2), cv2.NORMAL_CLONE) / 255.
